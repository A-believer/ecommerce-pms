<template>
  <div class="max-w-md mx-auto p-4 text-xs leading-3">
    <form @submit.prevent="submitForm" class="space-y-4">
      <base-spinner :is-loading="isLoading" text="Saving"></base-spinner>
      ...
      <div class="flex flex-col gap-y-2">
        ...
        <textarea
          ...
          id="description"
          class="px-2 py-2 w-full border border-gray-500 rounded-md resize-none"
          required
        ></textarea>
...
const submitForm = async () => {
  ...
  try {
    ...
    await productService.updateProduct(props.product?.id, {
      name: data.value?.name,
      description: data.value?.description,
      price: data.value?.price,
      category: data.value?.category,
      stock: data.value?.stock,
      image: data.value?.image ? imageUrl : props.product?.image
    })
